...
final class Proxifier
{
    ...
    /**
     * @var array<class-string, class-string<StabilityChecker>>
     */
    private $stabilityCheckers;

    /**
     * @param array<class-string, class-string<StabilityChecker>> $stabilityCheckers
     */
    public function __construct(ContainerBuilder $container, array $stabilityCheckers = [])
    {
    ...
    private function prepareServicePool(string $wrappedServiceId, Definition $serviceDef): Definition
    {
        ...
        $checkerClass = $this->stabilityCheckers[$serviceClass];
        $this->container->findDefinition($checkerClass);
        $svcPoolDef->setArgument(2, new Reference($checkerClass));
